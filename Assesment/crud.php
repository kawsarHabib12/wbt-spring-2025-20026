...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $action === "create") {
    ...
    if ($first === "" || $last === "" || $contact === "" || $email === "" || $password === "" ) {
        $error = "All fields are required and age must be a positive number.";
    } else {
        $stmt = mysqli_prepare(
            $conn,
            "INSERT INTO students (first_name, last_name, contact, email, password)
             VALUES (?, ?, ?, ?, ?)"
        );
        mysqli_stmt_bind_param($stmt, "sssss", $first, $last, $contact, $email, $password);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $action === "update") {
    ...
    if ($id <= 0 || $first === "" || $last === "" || $contact === "" || $email === "" || $password === "") {
        ...
        mysqli_stmt_bind_param($stmt, "sssssi", $first, $last, $contact, $email, $password, $id);
...
if ($action === "edit") {
    ...
    if ($id > 0) {
        $stmt = mysqli_prepare(
            $conn,
            "SELECT id, first_name, last_name, contact, email, password FROM students WHERE id = ?"
        );
        mysqli_stmt_bind_param($stmt, "i", $id);
